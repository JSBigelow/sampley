...
def absencelines_delimit(
    ...
    # absence lines (currently surveyed lines)
    check_dtype(par='sections', obj=sections, dtypes=gpd.GeoDataFrame)
    absencelines = sections.copy()[['section_id', 'datetime', 'geometry']]  # copy certain columns of sections
    ...
    if tm_threshold is not None and tm_unit is not None:  # if there is a temporal threshold and temporal unit
        ...
        presencezones = []  # list for presence zones
        for section_unit in absencelines['unit'].unique():  # for each unique unit of the absence lines
            ...
            if tm_unit.lower() in ['year', 'day']:  # if the specified temporal unit is year or day
                for presence_unit, presence_point in zip(presences['unit'], presences['point']):  # for each presence
                ...
                for presence_unit, presence_point in zip(presences['unit'], presences['point']):  # for each presence
                    inner_diff = max(section_unit, presence_unit) - min(section_unit, presence_unit)  # calculate the inner difference
                    # if the inner difference is less than or equal to the threshold or greater than or equal to the complementary...
                    if inner_diff <= tm_threshold or inner_diff >= tm_threshold_complementary:
                        presences_overlap.append(presence_point)  # ...append its geometry to the list
            presencezones.append({  # append to the presence zones list...
                'unit': section_unit,  # the unique unit of the absence lines
        ...
        absencelines = pd.merge(absencelines, pd.DataFrame(presencezones), on='unit', how='left')  # merge the presence zones to the absence lines
        absencelines['presencezones'] = gpd.GeoSeries(absencelines['presencezones'], crs=presences.crs)  # convert presence zones to GeoSeries
        absencelines.drop('unit', axis=1, inplace=True)  # remove unit col
        presences.drop('unit', axis=1, inplace=True)  # remove unit col
    else:  # else if there is no temporal threshold or unit, the presence zones are identical (and each contain all presences)
    ...
    absencelines['absencelines'] = (
        absencelines.apply(lambda r:  # take surveyed lines and presence zones and...
...
def absences_delimit(
    ...
    absencelines['dfbal'] = absencelines.geometry.length.cumsum()  # calculate the distance from beginning of absence lines
    ...
    while i < target:  # while count of absences is less than the target to be generated
        ...
        if var in ['a', 'along']:  # along-the-line variation - randomly sample point along absence lines
            ...
            #   ...making a line parallel to the tiny line at the specified distance on the randomly chosen side - parallel_offset()
